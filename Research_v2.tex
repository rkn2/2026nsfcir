...
single event size.
Extending the recession analysis across the gauge's full 1990--2026
instantaneous record strengthens this anchor: fifteen well-fit flood events give
$\hat{\tau}$ between 36 and 96 hours (median 51), an event-to-event spread of
$\mathrm{sd}(\ln \hat{\tau}) = 0.30$ that supplies the pre-estimate of
$\sigma_{\mathrm{MB}}$ in Subtask~1.2, and a quantified antecedent-moisture
signal: wetter pre-event conditions correlate with slower recession (a shift of
roughly 33 hours from the driest to the wettest antecedent state, with
pre-event baseflow as the predictor), so the antecedent state that pre-storm
mode must account for is itself observable from the same gauge the operational
...
A complementary sweep of that slope parameter against documented 2023 outcomes
(Main Street landmarks flooded, the State House dry) brackets the admissible
slope at 0.9\,m/km or above and predicts 234 inundated buildings at that lower
bound, the same order as the 271-building empirical set; the ICEYE depth field
pins the value precisely.
...
against the 2024 SAR duration brackets at or below the ranking-stability
threshold established by a pre-submission sensitivity analysis (archived with
the project repository): approximately 0.4 log-units, roughly 70 hours RMSE over
the building portfolio, below which the top-20 retrofit priority set retains at
least 80\% overlap with the error-free ranking across fragility parameter
variants; empirical consistency of 90\% credible intervals with
