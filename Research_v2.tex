...
use, with ICEYE commercial Synthetic Aperture Radar (SAR) providing building-level inundation duration and depth for one-time model calibration);
...
RO1 builds and validates the technical CPS by coupling four components: SAR imagery with ICEYE
building-level data processing, interpretation, and error characterization (Subtask~1.1), a
...
\textbf{Subtask 1.1: ICEYE Data Processing, Interpretation, and Likelihood Characterization
(lead: Wauthier).}
Subtasks 1.1 and 1.2 run in parallel in Year 1: PI Wauthier extracts building-level inundation measurements from ICEYE SAR while PI Busse develops the GIS-parameterized mass balance model; neither depends on the other's output until both feed into the Bayesian fusion framework in Subtask 1.3. 
...
SAR imagery from both Sentinel-1 and ICEYE will be used to characterize building-level inundation duration and depth across both the 2023 and 2024 events.
% christelle said : yes I would just say that we will use SAR imagery for both events including Sentinel - 1 and ICEYE. No need to explain the details of 2024 and that we might not get them right away --> so should we mention sentinel before this?????
...
% we could also use sentinel 1 in the workshops if it is useful for flood mapping since it is free FYI from christelle do we want to include that? i thought the resolution wasnt good enough? or is it fine we need to figure this out TODO
...
% christelle says mention sar imagery but i think the whole point is that we are using sar imagery to calibrate maggies models so that we wouldnt need sar imagery every time since we didnt have perfect ree data. TODO does this make sense
