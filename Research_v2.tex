...
deliberately simple cyber-layer component: peak gauge stage projected onto the
USGS 3DEP terrain model along a reach-scale water-surface slope (a stage-to-DEM
mapping), which
yields building-level peak depth $d_i$ and the inundation extent (a building is
predicted inundated where its projected depth is positive).
...
The 2023 ICEYE depth measurements calibrate the mapping's water-surface slope and
bound its building-level error in
...
per-scene wet/dry classification error, characterized by PI Wauthier in
Subtask~1.1, can be incorporated by softening the truncation boundaries without
...
$\delta_i^{\mathrm{comm}}$ of Equation~(2); the SAR brackets calibrate the error
...
The posterior duration distribution from Subtask~1.3 (Equation~(4)) propagates through
...
drives the $\delta_i^{\mathrm{comm}}$ adjustments in Equation~(2); community-engaged
...
calibrated duration distribution of Equation~(4) in Montpelier, or the
...
adjustments for at least 20\% of buildings, with zero defaults recorded as
explicit no-knowledge outcomes rather than counted as elicited values.
