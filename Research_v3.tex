...
the \textbf{physical layer} (load-bearing masonry and timber structures built before
modern building codes, absent from existing fragility frameworks);
...
with older building stock, enabling evaluation of building-specific hardening
...
That dead end has one exception.
...
Measuring that contribution, and the conditions under which it holds, is the
scientific core of the work that follows.
...
% PROPORTION TODO (2026-07-27): trim the material-science prose density below
% (mortar softening, brick saturation, rising damp) in the page-cut pass. This is
% the PHYSICAL layer of the CPS and must stay, but the detail here can make the
% proposal read as masonry science rather than community + CPS. Keep the equation,
% the 2-component design, and the 2023/2024 validation; compress the material
% detail so masonry reads as one layer serving the loop, not the subject.
