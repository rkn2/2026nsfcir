...
event-to-event anchor $\sigma_{\mathrm{event}}$ for the Subtask~1.3 calibration), and a quantified
...
\cite{hall2009water,franzoni2015compressive,sathiparan2018effect}; those crossing
times, evaluated at a reference depth, anchor the ordered $\lambda_{0,ds}$, the
rate at which deeper immersion (a greater saturated wall height) accelerates the
crossing anchors the shared $\gamma$, and the specimen-to-specimen scatter of the
published curves anchors the shared $\zeta$.
...
carrying no survivor term. Each $P(DS_i \geq ds \mid a)$ is computed per building
by Monte Carlo integration of Equation~\eqref{eq:fragility} over building $i$'s
own duration posterior $p(T_i \mid a)$ jointly with its calibrated depth
distribution from the Subtask~1.2 stage-to-DEM projection under the stage
scenario evaluated (for planning mode, a recurrence of the 2023 crest or a
design recurrence stage), with draws in which the building is not inundated
contributing no loss; the outer sum runs over buildings with nonzero inundation
probability under that scenario. The action $a$ modifies
both the fragility (building hardening raises the capacity threshold
$\lambda_{0,ds}$) and the duration posterior (drainage investment reduces
$\hat{\tau}_k$ through the mass balance model).
...
derived from the $\sigma_{\mathrm{MB}}^2$ characterization in Subtask~1.3
together with the elicitation-initialized adjustment uncertainty $P_i^{(0)}$, are
...
\item \textit{Technical KPIs} (Subtask~1.5):
  ...
  exceeding the depth-only baseline by a margin statistically significant under
  the cluster-bootstrap comparison of per-building predictive scores in
