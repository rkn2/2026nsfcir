...
These questions together span three scientific contributions, summarized in
Table~\ref{tab:research_summary}. First, a generalizable CPS architecture principle
demonstrating that free, nationally available geospatial data fused with community
knowledge is sufficient for the investment decisions small municipalities face.
Second, the physical layer existing frameworks omit, the first duration-dependent
fragility surfaces for the older load-bearing masonry these communities are built
from, so the cyber layer's duration output and the community's memory of how long
water sits translate into damage rather than being discarded by depth-only models.
Third, a characterization of the conditions under which locally held community
...
The \textit{physical component} grounds the duration claim in the material science of
how immersion time degrades older masonry, chiefly mortar softening, brick
saturation, and rising damp, which sets the functional form of the duration effect
independently of observational confounding.
...
Published saturation-strength and capillary-absorption curves for historic brick and lime mortar \cite{hall2009water,franzoni2015compressive,sathiparan2018effect} set these priors, with the immersion time at which strength loss crosses each damage state anchoring the ordered $\lambda_{0,ds}$, its acceleration with depth anchoring the shared $\gamma$, and specimen-to-specimen scatter anchoring the shared $\zeta$.
