...
Closing the loop requires four capabilities. The first is estimating how long
water stays at building level. The second is translating duration into structural
damage for the affected building stock. The third is calibrating with community
knowledge where data are sparse. The fourth is converting damage probabilities
into investment decisions whose outcomes feed back into the next event. Each has
a mature literature. None addresses the interface with the next.

The broader CPS literature does not close it either. Community sensing
architectures for flood monitoring assume local sensor infrastructure, proprietary
data contracts, or sustained community effort that resource-constrained
municipalities cannot maintain after a grant ends, and none carries the community
knowledge mechanism, independent ground truth, or physical damage model the loop
requires.
...
(Fig.~\ref{fig:damage_map}).
% becca: add the actual counts per damage state when ready (how many no damage / cosmetic / structural).

\begin{figure}[h]
\vspace{-0.2cm}
\centering
\includegraphics[width=0.85\textwidth]{images/damage_map_2023.pdf}
\captionsetup{font=small}
\vspace{-0.3cm}
\caption{\hl{\textbf{PLACEHOLDER --- BECCA VERIFY AGAINST ACTUAL FIELD DATA.}}
Damage state by building footprint, Montpelier downtown core, July 2023 field
assessment. Three ordered states: no damage (green), cosmetic (yellow), structural
(red). The 271 pre-code masonry buildings used in the fragility analysis are a
subset of the footprints shown. Synthetic assignments based on river proximity;
replace with actual field observations before submission.}
\label{fig:damage_map}
\vspace{-0.3cm}
\end{figure}
...
the flooded core by 0.8 to 1.5\,m, implying a water-surface slope of 0.6 to
1.0\,m/km, the single parameter the 2023 ICEYE depth field calibrates
(Subtask~1.2).
...
RO1 (Napolitano, Wauthier, Busse) addresses RQ1 through five coupled subtasks (1.1 to 1.5); RO2 (Napolitano with all PIs) addresses RQ2 through five (2.1 to 2.5). Together the two objectives supply the interfaces that the state of the art leaves missing and close the loop end to end. Leads and dependencies are given in each subtask header below and in the Project Timeline section.

The loop closes at three timescales rather than through continuous automated actuation. Hours before a flood peak, NOAA forecasts drive pre-storm resource staging. After each event, community priors and the error model update from what the flood revealed. Over years, retrofit decisions feed back through the $\hat{\tau}_k$ and fragility updates of Subtask~2.4, changing the flood behavior the system models next. Human decisions are the actuators at every timescale, so the community is a functional component of the control loop rather than a recipient of its outputs, the connected-communities reading of CPS the CIR track invites. The research questions emerged from PI Napolitano's post-2023 Montpelier engagement, whose partners' question, where limited flood-mitigation dollars belong, defined the decision problem.
...
partners a three-mode decision interface and transferable protocol for independent
community use.}
\end{tcolorbox}

% FRAMING SWEEP TODO: confirm each RO2 subtask reflects the updated RO2 box —
% specifically that the substitute-versus-complement characterization (now in the
% box) is carried through Subtasks 2.1–2.5, not just 2.3 where it currently lands.
% Also flag line ~921 (Subtask 2.2 Year 2): "cross-references community-observed
% damage against 2023 field damage states" — against a mostly-damaged binary
% baseline this check may be near-uninformative; revisit when the ordinal taxonomy
% is finalized.
...
transferable protocol (Subtask~2.5) documented and released with characterized
interval widths for communities without local SAR calibration.
\end{itemize}
