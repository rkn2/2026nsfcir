...
framework fusing a GIS-parameterizable mass balance model with community-elicited
duration priors and a gauge-stage-to-terrain projection for building-level depth; and
the community closes the loop through a co-developed three-mode interface for retrofit
...
These questions span three coupled contributions, mapped to their gaps and outcomes
in Table~\ref{tab:research_summary}. The first is \textbf{computational}: a Bayesian
framework that infers building-level inundation depth and duration from sparse,
indirect, interval-censored observations (gauge stage projected across terrain, SAR
duration brackets), fuses them with a GIS-parameterizable mass balance model and
community-elicited priors, and propagates uncertainty to a decision-adequate damage
estimate without hydraulic simulation. The second is \textbf{physical}: the first
duration-dependent fragility surfaces for the older load-bearing masonry these
communities are built from, identified under depth-duration collinearity through
material-science priors, so the framework's duration output registers as damage
rather than being discarded by depth-only models. The third, and the defining
novelty, is treating locally held \textbf{community knowledge} as a structural
calibration input: a characterization of when knowledge entered as Bayesian priors
improves accuracy and calibration over a GIS-only baseline, and where it functions as
a substitute versus a complement to geospatial data. Together these establish a
generalizable principle, that free national data fused with community knowledge is
sufficient for the investment decisions small municipalities face, transferable to
any domain with national data and community knowledge but no simulation capacity.
...
\hline
\endlastfoot
\textbf{Computational:} Whether a GIS-parameterized mass balance model can produce
...
\hline
\textbf{Physical:} Duration-depth-damage relationships for pre-code
load-bearing masonry are uncharacterized; existing fragility frameworks assume
depth as the governing input and cannot serve a computational core that produces
duration &
...
beyond depth \\
\hline
\textbf{Community knowledge:} Community knowledge is structurally excluded from
...
\hline
\textbf{Decision interface:} No existing interface makes probabilistic duration-based
...
\end{tabularx}
\vspace{2pt}
\fontsize{10}{12}\selectfont\textit{\textbf{Data inputs} (underpin all rows):
...
context. No research gap. This reliance on free, national inputs is a deliberate
design choice.}
\endgroup
\normalsize
...
proposal's free, national inputs, USGS stream gauges, NOAA precipitation, and SAR
imagery, make both tests tractable and transferable to any gauged municipality, with
community knowledge entering the computational core as measurable Bayesian priors
rather than supplementary narrative.
...
the CPS decision interface with community partners in Montpelier; characterize the
