...
These questions span coupled contributions across the CPS layers, mapped to their
gaps and outcomes in Table~\ref{tab:research_summary}. The \textbf{cyber layer} is
the computational core, a Bayesian framework that infers building-level inundation
...
uncertainty to a decision-adequate damage estimate without hydraulic simulation. The
\textbf{physical layer} contributes the first duration-dependent fragility surfaces
for the older load-bearing masonry these communities are built from, identified under
depth-duration collinearity through material-science priors, so the cyber layer's
duration output registers as damage rather than being discarded by depth-only models.
The \textbf{community layer} is the defining novelty, a characterization of when
locally held knowledge entered as Bayesian priors improves accuracy and calibration
over a GIS-only baseline, and where it functions as a substitute versus a complement
to geospatial data. Together these establish a generalizable CPS architecture
principle, that free national data fused with community knowledge is sufficient for
the investment decisions small municipalities face, transferable to any domain with
national data and community knowledge but no simulation capacity.
