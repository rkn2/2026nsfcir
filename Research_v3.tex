...
Extended inundation degrades these buildings through mortar softening, masonry
saturation, and rising-damp cohesion loss, none of which current assessment tools
observe or model
...
investment decisions small municipalities face, including pre-storm decisions made
before water rises; the first empirically derived, duration-dependent fragility
surfaces for pre-code load-bearing masonry archetypes, addressing existing
frameworks' structural incompatibility with a CPS cyber layer that produces duration
rather than depth estimates; and a characterization of the conditions under which
locally held community knowledge, entered as Bayesian priors, improves model accuracy
and calibration relative to a GIS-only baseline, and where community knowledge and
geospatial data function as substitutes versus complements in CPS calibration.
...
\end{tabularx}
\vspace{2pt}
\fontsize{10}{12}\selectfont\textit{\textbf{Sensing layer} (underpins all rows): USGS stream
...
To test whether the mass balance model's drainage timescale is recoverable from
free gauge data at the accuracy the fragility layer requires, we ran a recession
analysis of USGS gauge 04286000 (Winooski River at Montpelier; 397\,mi\textsuperscript{2})
for both events, using 15-minute discharge records (Fig.~\ref{fig:recession}):
...
gauge record alone, the quantity Subtask~1.2 disaggregates to sub-basin scale from
national GIS layers.
Second, the 2023 recession departs from a single exponential beyond roughly
60 hours ($R^2$ falls to 0.73 over the full falling limb), consistent with a
channel-to-storage drainage transition: local standing water in low-lying
sub-basins outlasts the river recession, motivating sub-basin $\hat{\tau}_k$
estimates over a single watershed constant.
...
(median 51), $\mathrm{sd}(\ln \hat{\tau}) = 0.30$ across events (the
$\sigma_{\mathrm{MB}}$ pre-estimate for Subtask~1.2), and a quantified
antecedent-moisture signal: wetter pre-event conditions correlate with slower
recession (roughly a 33-hour shift from driest to wettest antecedent state,
predicted by pre-event baseflow), so the antecedent state pre-storm mode must
account for is observable from the same gauge the operational system already
reads.
Discharge stayed above twice pre-event baseflow for roughly 19 days in 2023 and
10 days in 2024, bounding the window within which building-level standing water
drained, consistent with the multi-day inundation persistence in the framework's
scope conditions.
...
Building-level standing water, the quantity $T_i$ the framework models and the
SAR brackets measure, thus outlasted river-stage exceedance by days in 2023,
exactly the separation the sub-basin timescales $\hat{\tau}_k$ are designed to
capture and why the scope conditions are defined on standing-water persistence
rather than river-stage duration.
A first prototype of the Subtask~1.2 stage-to-DEM projection (1\,m 3DEP terrain;
gauge datum 499.87\,ft NAVD88) tests the depth pathway: projecting the 2023
crest as a flat water surface reproduces river-corridor inundation through
downtown ($\sim$92 hydraulically connected acres) but underpredicts the flooded
core, with Main Street landmarks 0.8 to 1.5\,m above the projected surface,
1.2 to 1.5\,km upstream of the gauge, consistent with a water-surface slope of
roughly 0.6 to 1.0\,m/km, the single parameter the 2023 ICEYE depth field
calibrates.
A complementary sweep of that slope against documented 2023 outcomes (Main
Street flooded, the State House dry) brackets the admissible slope at
0.9\,m/km or above, predicting 234 inundated buildings at that lower bound,
the same order as the 271-building empirical set that the ICEYE depth field
pins precisely.
...
An archived pre-submission cadence simulation shows that scenes spaced across the multi-day drainage window contribute substantially more calibration information than additional scenes clustered near the flood peak; this spacing criterion guides which acquisitions are prioritized for processing.
...
Subtask~1.3; a recession analysis of fifteen well-fit flood events in the
gauge's 1990--2026 instantaneous record (archived with the project repository)
gives $\mathrm{sd}(\ln \hat{\tau}) = 0.30$ as that pre-estimate.
...
estimate at every cadence at a bias cost bounded by the pre-estimate's own
error.
...
\textbf{Key outcomes:} RO1 produces three primary scientific outputs, detailed with
approaches and outcomes in Table~\ref{tab:research_summary}: a validated Bayesian
framework with characterized decision-adequacy across all three operational modes
(Subtask~1.5); the first empirically derived, duration-dependent fragility surfaces
for pre-code load-bearing masonry archetypes (Subtask~1.4); and a characterization
of the conditions under which the GIS-parameterized system is and is not
decision-adequate, including the range of forecast lead times over which pre-storm
predictions remain actionable (Subtask~1.5).
...
\textbf{Key outcomes:} RO2 produces three primary outputs, detailed in
Table~\ref{tab:research_summary}: a three-mode decision interface independently
operated by Montpelier community partners in Year~3 (Subtask~2.2); a direct
characterization of the conditions under which community knowledge and geospatial
data are substitutes versus complements, including cases where community knowledge
adds no value (Subtask~2.3); and a transferable co-design protocol enabling any
municipality with a USGS gauge to initialize the system from one workshop without
a commercial SAR purchase (Subtask~2.4).
...
\begin{figure}
\includegraphics[width=0.5\textwidth]{images/img.jpg}
\captionsetup{justification=centering,singlelinecheck=false,font=small}
...
\item \textit{Technical KPIs} (Subtask~1.5):
  ...
  \item Building-level duration estimation error against the 2024 SAR duration
  brackets at or below the ranking-stability threshold established by a
  pre-submission sensitivity analysis (archived with the project repository):
  approximately 0.4 log-units, $\sim$70 h RMSE over the building portfolio, below
  which the top-20 retrofit priority set retains at least 80\% overlap with the
  error-free ranking across fragility parameter variants.
  \item Empirical consistency of 90\% credible intervals with the observed
  brackets between 0.85 and 0.95 across 271 buildings.
  \item Damage state prediction accuracy of the duration-depth bivariate model
  exceeding the depth-only baseline by a statistically significant margin under
  cross-validation on 2023, with the same comparison reported on the 2024
  held-out event relative to the damage variation that smaller event produced.
...
\item \textit{Community engagement KPIs} (Subtask~2.1):
  \begin{itemize}[leftmargin=1.5em,itemsep=1pt,topsep=1pt]
  \item Proportion of Year~1 stated community priorities reflected in Year~2
  framework outputs, evaluated by community partners in Year~2 workshops with
  all discrepancies documented and addressed before Year~3.
  \item Structured elicitation sessions covering all 271 buildings by end of
  ...
  recorded as explicit no-knowledge outcomes rather than counted as elicited
  values.
  \end{itemize}
...
\item \textit{Adoption KPIs} (Year~3): emergency managers successfully operate all
three decision interface modes without researcher support in the Year~3 stress
test, and demonstrate ability to explain pre-storm risk maps and planning-mode
retrofit comparisons to elected officials and residents without researcher
involvement.
\end{itemize}
