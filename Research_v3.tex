...
Four literature domains each supply one piece of the loop this proposal closes: a
hydrological prior on how long water stays, a mechanism for entering community
knowledge as quantified belief, a physical damage model for the affected building
stock, and a decision framework for allocating mitigation dollars. None couples them,
so the loop has never been evaluated end to end. No prior work runs free national data
and community knowledge through a physical model of pre-code building stock to a damage
estimate a municipality acts on, then feeds those actions and each new event back into
the model. That is what RQ1 and RQ2 test, arbitrated by real flood outcomes across 271
buildings with sensor-measured durations and field-recorded damage states.

The broader CPS literature does not close it either. Community sensing architectures
...
cannot maintain after a grant ends, and none carries the community knowledge mechanism,
independent ground truth, or physical damage model the loop requires. This
proposal's free national inputs, USGS stream gauges, NOAA precipitation, and SAR
imagery, make it tractable and transferable to any gauged municipality, with
...
quantitative calibration input, and decision frameworks connect neither upstream
drainage investment nor community knowledge to building damage probabilities.
