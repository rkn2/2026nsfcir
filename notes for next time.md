...
## PICK UP HERE NEXT TIME (2026-07-28, part 2)
Reworked the **intellectual-merit paragraph** (RQ subsection, ~line 177) so the **closed loop is the headline contribution** and the three advances (computational / physical / community knowledge) are subordinate to it, not co-equal novelties. Key moves, all Becca's calls this session:
- Opens with what it *is* (the integrated CPS loop), not what it is not.
- **Physical contribution deemphasized masonry hard**: it now reads as the two-sided coupling to the physical environment (sensing flood signatures; interventions like drainage/hardening whose effects reenter future flood behavior). The fragility-surface clause was **removed entirely** from this paragraph (masonry detail lives in Subtask 1.4). Duration still appears via the computational sentence.
- "The third, and the defining novelty" → plain "The third is..." (dropped LLM-ish "sharpest" too).

**NEXT TIME — consistency sweep (Becca's explicit request):** make the rest of the document match this reframe. Two things to check document-wide:
1. **How much masonry/the "physical layer" is deemphasized** — the merit paragraph now treats masonry as one mechanism serving the loop, not a headline. Check Subtask 1.4 prose density (already flagged for a proportion trim, `%` marker at its start) and any spot that still elevates masonry/fragility to a co-equal contribution.
2. **Closed-loop framing** — the merit paragraph now leads on the integrated loop as THE novelty. Check that abstract/summary, goals box, broader impacts, and RQ lead-ins all speak the same "closed loop / integration is the contribution" language, not the older "three contributions" or "four-layer" phrasing.
