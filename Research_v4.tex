...
Building that measurement into a working loop is the work that follows.
...
\textbf{physical}: the CPS senses flood signatures and informs physical
interventions (drainage retrofits, building hardening) whose effects reenter
future flood behavior. The third treats \textbf{community knowledge} as a
...
Community knowledge constitutes a substantial information resource that formal
...
physical model of the building stock, so nothing carries a completed retrofit back
into future flood behavior, and none allows drainage and building hardening to be
compared on the same probabilistic basis.
None is designed for small municipalities; the hazard mitigation planning
literature documents that plan quality falls systematically with municipal size
\cite{smith2013statecapacity,horney2012ruralstatus,smith2026ruralcapacity}.
The fragility gap propagates directly: none addresses pre-code masonry or
duration-dependent damage states, none provides building-level predictions from
forecast precipitation for pre-storm staging, and no framework improves as a
...
Outputs are per-building brackets and peak depths for both events with per-scene classification error rates; comparing mass balance predictions against the 2023 brackets calibrates $\sigma_{\mathrm{MB}}^2$ in Subtask~1.3. Cadence simulation shows scenes spread across the drainage window carry more calibration information than scenes clustered near the peak.
...
twenty across the flood-exposed core; terrain-partition sensitivity analysis
confirms this sits inside the plateau where finer partitioning adds degrees of
...
as an order-of-magnitude anchor. Sensitivity analysis confirms the calibration is
...
a smooth, hydraulically connected profile; slope sensitivity analysis bounds the
...
cadence makes brackets tight. The 271-building dataset constitutes one draw from
...
buildings. Left: 100-year flood baseline (no action). Center: same scenario
with the proposed upstream retention basin, showing reduced structural damage.
Right: NWS 48-hour forecast (17.2\,ft crest) with pre-storm staging priorities.
...
resolves an allocation problem that annually re-injures small historic
municipalities. The generalizable principle, that free national data fused with
tacit process knowledge suffices for the investment decisions these municipalities
face, extends to any infrastructure domain with national data and practitioner
knowledge but no simulation capacity. The substitutes-versus-complements
